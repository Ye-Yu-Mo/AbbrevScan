...
interface Translations {
  ...
  snakeGameScore: string;
  snakeGameOver: string;
  snakeFinalScore: string;
  snakeRestart: string;
  snakePaused: string;
  snakeContinue: string;
  snakeInstructionsTitle: string;
  snakeInstruction1: string;
  snakeInstruction2: string;
  snakeInstruction3: string;
  snakeInstruction4: string;
  snakeProcessing: string;
}
...
const SnakeGame: React.FC<SnakeGameProps> = ({ t }) => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [score, setScore] = useState(0);
  const [gameOver, setGameOver] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const gameLoopRef = useRef<NodeJS.Timeout | null>(null);
  const snakeRef = useRef<Position[]>([{ x: 10, y: 10 }, { x: 9, y: 10 }, { x: 8, y: 10 }]); // Initial length: 3
  const directionRef = useRef<Position>({ x: 1, y: 0 }); // Start moving right
